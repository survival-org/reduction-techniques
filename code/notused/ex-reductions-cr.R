...
source("code/functions/get-cif-from-haz-df.R")
...
ndf_cr_xgb_pem <- ndf_cr |> cbind(haz_xgb_pem = haz_xgb_pem[["response"]]) |> 
  group_by(pneu, cause) |>
  add_cumu_hazard(pam) |>
  mutate(cumu_haz_xgb_pem = cumsum(haz_xgb_pem*intlen))

ndf_cr_xgb_pem_long <- ndf_cr |> 
  cbind(haz_xgb_pem = haz_xgb_pem[["response"]]) |> 
  group_by(pneu, cause) |>
  add_cumu_hazard(pam) |> 
  mutate(cumu_haz_xgb_pem = cumsum(haz_xgb_pem * intlen)) |> 
  ungroup() |> 
  select(-haz_xgb_pem) |> 
  pivot_longer(
    cols = c(cumu_hazard, cumu_haz_xgb_pem),
    names_to = "method",
    values_to = "cumu_hazard"
  ) |> 
  mutate(
    method = recode(method,
                    cumu_hazard = "PAM",
                    cumu_haz_xgb_pem = "XGBOOST")
  )

# compare hazards
ggplot(ndf_cr_xgb_pem_long, aes(x=tend, lty= pneu, col = method)) + 
  geom_line(aes(y=cumu_hazard)) + 
  facet_wrap(~cause)
...
ndf_cr_xgb_pem <- ndf_cr_xgb_pem |>
  ...
  group_by(pneu, cause) |> 
  mutate(cumu_hazard = cumsum(haz_xgb_pem)) |>
...
ggplot(ndf_cr_xgb_pem, aes(x=tend, lty= pneu)) + 
