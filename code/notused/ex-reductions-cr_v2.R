...
lrn_rf_dt = po("encode", method = "treatment") %>>%
  lrn("classif.ranger", 
      ...
      min.node.size = 1) |> 
  as_learner()
# rn_rf_dt = po("encode", method = "treatment") %>>% 
#   lrn("classif.xgboost", objective = "binary:logistic", nrounds = 1000) |> as_learner()
...
ggplot(ndf_cr_dt, aes(x=tend, y = cif)) +
  geom_line(aes(col = pneu)) +
  facet_wrap(~cause)

ndf_cr_rf_dt <- ndf_cr_dt_interim |>
  rename(haz_rf_dt = hazard) |>
  add_hazard(dt, ci=FALSE)
# build new data frame including cif for pam
ggplot(ndf_cr_rf_dt, aes(x = tend, y = hazard)) + 
  geom_line() + 
  geom_line(aes(y = haz_rf_dt), col = "red") + 
  facet_grid(cause ~ pneu)

# ---------------------------------------------------------------------------- #
# add trans prob approach
# ---------------------------------------------------------------------------- #

ndf_cr_xgb_pem <- ndf_cr_xgb_pem |>
  mutate(transition = ifelse(cause == 1, "1->2", "1->3")) |>
  group_by(pneu, transition) |> 
  rename(cumu_hazard = cumhaz)|>
...
ggplot(ndf_cr_xgb_pem, aes(x = tend, y = trans_prob, lty=pneu)) + 
  geom_line() + 
  facet_grid(~ cause)
...
ndf_cr_xgb_pem <- ndf_cr_xgb_pem |> 
  group_by(pneu) |> 
  arrange(pneu, tend) |> 
  add_cif_from_haz(haz_col = "haz_xgb_pem")

ggplot(ndf_cr_xgb_pem, aes(x = tend, y = cif, lty=pneu)) + 
  geom_line() + 
  facet_grid(~ cause)

ndf_cr_rf_dt <- ndf_cr_rf_dt |>
  mutate(transition = ifelse(cause == 1, "1->2", "1->3")) |>
  group_by(pneu, transition) |> 
  rename(cumu_hazard = cumhaz_rf_dt)|>
  add_trans_prob(dt, ci = FALSE) |> # object not used because ci = FaLSE and cumu_hazard in data set
  ungroup() |>
  mutate(
    cause = factor(cause, labels = c("Death", "Discharge"))
    , pneu = factor(pneu, labels = c("Pneumonia", "No Pneumonia"))
    , model = "Random Forest DT"
  )

ggplot(ndf_cr_rf_dt, aes(x = tend, y = trans_prob, lty=pneu)) + 
  geom_line() + 
  facet_grid(~ cause)
